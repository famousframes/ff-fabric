...
export class EditorComponent implements OnInit {
  ...
  removeSelectedColorSwatch() {
    if(this.palettes.selected.type === undefined) {
      let _id = this.palettes.selected.id;
  ...
  addToCustomPalette(type: string) {
    switch (type) {
      case 'fill':
        this.palettes.custom.push({key: this.props.fill, value: this.props.fill, id: this.palettes.custom.length});
        break;
      case 'stroke':
        this.palettes.custom.push({key: this.props.stroke, value: this.props.stroke, id: this.palettes.custom.length});
        break;
    }
    this.savePalette();
  }
  ...
  savePalette() {
    let json = JSON.stringify(this.palettes.custom);
    localStorage.setItem('ffFabricCP', json);
  }

  loadPalette() {
    let palettes = localStorage.getItem('ffFabricCP');
